...
<!-- ============================================================
     HERO
     ============================================================ -->
<section class="relative bg-[#0f1c2e] overflow-hidden">
  ...
  <div class="absolute inset-0 bg-gradient-to-b from-[#0f1c2e]/80 to-[#0f1c2e]"></div>
  ...
  <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-24">
    <div class="max-w-3xl">
      <div class="inline-flex items-center gap-2 bg-[#d4a843]/15 border border-[#d4a843]/30 rounded-full px-4 py-1.5 mb-6">
        ...
        <span class="text-[#d4a843] text-xs font-semibold tracking-widest uppercase">Project Gallery</span>
      </div>
      <h1 class="text-4xl sm:text-5xl xl:text-6xl font-bold text-white leading-[1.05] tracking-tight mb-6">
        ...
        <span class="text-[#d4a843]"> Across the Southeast.</span>
      </h1>
      <p class="text-gray-300 text-lg leading-relaxed mb-3 max-w-2xl">
...
<div class="bg-white border-b border-gray-100 sticky top-0 z-40 shadow-sm">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex items-center gap-1 overflow-x-auto py-4 scrollbar-hide -mx-1 px-1">
      ...
      foreach ($filters as $filter) : ?>
        <button
          ...
          class="ig-filter flex-shrink-0 text-xs font-semibold px-4 py-2 rounded-full border transition-all duration-200 whitespace-nowrap
                 <?php echo $filter['key'] === 'all'
                   ? 'bg-[#0f1c2e] border-[#0f1c2e] text-white'
                   : 'bg-white border-gray-200 text-gray-500 hover:border-[#0f1c2e]/30 hover:text-[#0f1c2e]'; ?>">
...
<!-- ============================================================
     GALLERY GRID
     ============================================================ -->
<section class="bg-[#f7f5f0] py-12 lg:py-16" id="gallery-grid-section">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

    <!-- Results count -->
    <div class="flex items-center justify-between mb-6">
      <p class="text-sm text-gray-500">
        Showing <span id="ig-visible-count" class="font-semibold text-[#0f1c2e]">0</span> projects
      </p>
    </div>
    ...
    <div id="ig-gallery-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
       ...
       * PHOTO DATA ARRAY
       * ─────────────────────────────────────────────────────────────
       * When real photos arrive from the client:
       *   - Replace 'img' with the actual filename in /images/gallery/
       *   - Update 'title', 'location', 'sector' per caption template
       *   - Add or remove items freely — the filter logic is data-driven
       * ─────────────────────────────────────────────────────────────
       */
      $gallery_items = [
      ...
      ?>
        <div class="ig-card <?php echo esc_attr($col_span); ?> group relative overflow-hidden rounded-xl bg-gray-200 cursor-pointer"
             data-category="<?php echo esc_attr($item['cat']); ?>"
             style="aspect-ratio: <?php echo !empty($item['span']) ? '2/1' : '1/1'; ?>;">
          ...
          <div class="absolute top-3 left-3">
            <span class="bg-[#0f1c2e]/80 backdrop-blur-sm text-[#d4a843] text-[10px] font-semibold tracking-wide uppercase px-2.5 py-1 rounded-full">
              <?php echo esc_html($cat_label); ?>
            </span>
          </div>

          <!-- Hover overlay -->
          <div class="absolute inset-0 bg-gradient-to-t from-[#0f1c2e]/90 via-[#0f1c2e]/40 to-transparent
            ...
            <div class="flex items-center gap-2 flex-wrap">
              <span class="flex items-center gap-1 text-gray-300 text-xs">
                <svg class="w-3 h-3 text-[#d4a843]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span class="text-gray-400 text-xs"><?php echo esc_html($item['sector']); ?></span>
            </div>
          </div>
    ...
    <div id="ig-empty-state" class="hidden py-24 text-center">
      ...
      <p class="text-gray-400 text-sm">No projects found for this category.</p>
    </div>
...
<section class="bg-[#0f1c2e] py-16 lg:py-20">
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
    <p class="text-xs font-semibold tracking-widest text-[#d4a843] uppercase mb-4">Work With Us</p>
    <h2 class="text-3xl sm:text-4xl font-bold text-white leading-tight mb-4">
      Ready to See This Quality on Your Next Project?
    </h2>
    <p class="text-gray-400 text-base leading-relaxed mb-8 max-w-xl mx-auto">
    ...
    <a href="/contact-us"
       class="inline-flex items-center gap-2 bg-[#d4a843] hover:bg-[#c49a38] text-[#0f1c2e] font-bold px-8 py-4 rounded transition-colors duration-200 text-sm tracking-wide">
...
(function () {
  ...
  function updateCount() {
    const visible = document.querySelectorAll('.ig-card:not([style*="display: none"])').length;
  ...
  function applyFilter(key) {
    cards.forEach(function (card) {
      ...
      // For col-span-2 cards: preserve span only when visible
      if (card.classList.contains('col-span-2')) {
        if (!show) {
          card.dataset.hadSpan = 'true';
        }
      }
    });
    updateCount();
